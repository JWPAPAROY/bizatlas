...
**등급은 출처가 아니라 실체 확인 여부로 정한다.** tier 를 파이프라인으로만 정하면 OpenAI·Airbnb·Ford
처럼 명백히 검증된 회사가 "TechCrunch 로 들어왔다"는 이유만으로 미검증 칸에 갇힌다.
`seed-proven` 의 **승격 모드**가 기존 `emerging` 을 Wikidata 로 재검증해 올린다 — 축 태깅은 이미
되어 있으므로 **AI 를 한 번도 쓰지 않는다**(쿼터 0). 2026-08-11 첫 실행에서 12건이 올라왔다.

```bash
curl -X POST .../functions/v1/seed-proven -H "x-ingest-secret: ..." -d '{"mode":"promote","limit":80}'
```

승격은 재현율보다 **정밀도**를 택한다(검증된 모델 칸이 이 서비스의 신뢰 기준선이라서).
3관문에 더해 **동일 회사 확증**을 요구한다: 도메인이 양쪽에 다 있으면 도메인 일치, 없으면 표제어 완전 일치.
이때 `website` 가 기사 URL 인 행(위 fallback 버그)은 도메인 근거로 쓰지 않는다 — 그대로 두면
`techcrunch.com ≠ openai.com` 이라며 멀쩡한 회사를 전부 떨군다.
...
- **Gemini 무료 티어 한도는 "모델별 하루 20회"다** (quotaId `GenerateRequestsPerDayPerProjectPerModel-FreeTier`).
  ...
  모델마다 쿼터 버킷이 다르므로 `GEMINI_MODELS` 폴백 체인으로 용량을 늘린다 (5개 × 20 ≈ 하루 100건).
  **별칭은 실제 모델과 버킷을 공유한다** — `gemini-flash-latest` = `gemini-3.6-flash`. 둘 다 넣어도 소용없다.
  `gemini-3.1-flash-lite-preview` 도 `gemini-3.1-flash-lite` 와 같은 버킷이다.
  체인에서 뺀 것: `gemma-4-*`(system_instruction·JSON 모드 무시), `gemini-2.5-*`·`gemini-2.0-*`(퇴역).
  `gemini-3.5-flash` 는 과거 깨진 JSON 때문에 뺐다가 **2026-08-11 재검증 후 다시 넣었다**(3회 연속 정상).
  버킷 확인법: 200 응답의 `modelVersion`. `ListModels` 응답에 있어도 실제로는 404 인 모델이 있으니
  (2026-08 기준 `gemini-2.5-flash`) 목록만 믿지 말고 실제 요청 형태로 찔러볼 것.
- **모델 퇴역(404)은 429 와 똑같이 다음 모델로 넘겨야 한다.** 2026-08-11 에 `gemini-2.0-*` 두 개가
  404 로 사라졌는데, 404 가 폴백 분기에 안 걸려 있어서 체인이 죽은 모델에 갇힌 채 남은 항목을
  전부 즉시 실패시켰다(재평가 118건 중 79건). 다섯 파이프라인 모두 404 를 폴백 조건에 넣어뒀다.
...
- **한 번 판정한 항목은 `seen_items` 에 남겨 다시 AI 에 태우지 않는다.** 반려된 것도 남긴다.
  **JSON 파싱 실패는 반려가 아니라 재시도로 처리하되, `MAX_ATTEMPTS`(2회) 상한을 둔다.**
  모델이 잘못 뱉은 것이지 항목의 문제가 아니라 바로 반려하면 멀쩡한 항목이 버려지는데,
  반대로 상한이 없으면 구조적으로 처리 불가능한 항목(본문이 길어 응답이 잘리는 기사 등)이
  매 실행마다 영원히 AI 를 태운다 — 실측으로 하루 12~18회, 전체 예산의 10~15% 가 여기서 샜다.
